...
/**
 * This class holds the layout of the game, including the locations of the game
 * elements like grid cells and towers/enemies. It also call on these objects to
 * update their location and behaviors, while functioning as a controller to
 * tell objects what to do, such as telling towers what enemies are within
 * range.
 * 
 * @author Michael Yang
 * @author Qian Wang
...
public class Layout implements Updateable {
	...
	private final Collection<BiConsumer<GameElement, GameElement>>[] nullActions = (Collection<BiConsumer<GameElement, GameElement>>[]) new Collection[2];
	...
	public Layout(List<Sprite> myNodes) {
		...
		// TODO: Get environment map from front end to load into GroovyEngine,
		...
		makeActionManager(new HashMap<String, String>(),
	 ...
	 */
	// TODO Poor design to have a method for every kind of sprite, need to think
	// of a better way to
	...
	/**
	 * Method for taking a collection of modifier maps from front end and
	 * translating them into lambda functions accessible by actionManager
	 * 
	 * @param modifiers
	 *            Modifier map from ModiferStrip.java frontEnd in authoring
	 *            environment
	 */
	public void modifiableHandler(Collection<Modifier> modifiers) {
		for (Modifier modifier : modifiers) {
			String[] tagPair = { modifier.getActor(), modifier.getActee() };
			Collection<BiConsumer<GameElement, GameElement>>[] actions = (Collection<BiConsumer<GameElement, GameElement>>[]) new Collection[2];
			Collection<BiConsumer<GameElement, GameElement>> action1 = new ArrayList<BiConsumer<GameElement, GameElement>>();
			Collection<BiConsumer<GameElement, GameElement>> action2 = new ArrayList<BiConsumer<GameElement, GameElement>>();
	...
	/**
	 * Method to create action manager from front-end user defined scripts and
	 * an interactionMap based on those scripts
	 */
	public void makeActionManager(Map<String, String> definedScripts,
			Map<String[], List<Integer>[]> interactionMap) {
		List<BiConsumer<GameElement, GameElement>> actionList = new ArrayList<BiConsumer<GameElement, GameElement>>();
		definedScripts.keySet()
				.forEach(
						s -> myGroovyEngine.addScriptToEngine(s,
								definedScripts.get(s)));
		definedScripts.keySet().forEach(
				t -> actionList.add((s1, s2) -> myGroovyEngine.applyScript(t,
						s1, s2)));
	...
	private void updatePathTest(GameElement e) {
		...
		int[] startIndices = myGameMap.getRowColAtCoordinates(
				enemy.getLocationX(), enemy.getLocationY());
		int[] endIndices = myGameMap.getRowColAtCoordinates(
				myGoalCoordinates[1], myGoalCoordinates[0]);
		try {
			enemy.planPath(grid, startIndices[0], startIndices[1],
					endIndices[0], endIndices[1]);
	...
	/**
	 * Creates a specific map and updates the current game map object in the
	 * layout. The specified GUID of the map name is used to know which map to
	 * instantiate. An array of gridcells are made and added to a map object,
	 * which is then set as the current map.
	 * 
	 * @param mapID
	 *            GUID of the map
	 */
	public void setMap(String mapID) {
		myGameMap = (GameMap) myGameElementFactory.getGameElement("GameMap",
				mapID);
		...
		for (String pathID : myGameMap.getPathNames()) {
			myPaths.put(pathID, (MapPath) myGameElementFactory.getGameElement(
					"MapPath", pathID));
		}
		Rectangle bounds = new Rectangle(myGameMap.getCoordinateHeight(),
				myGameMap.getCoordinateWidth());
		myCollisionChecker.initializeQuadtree(bounds);
	}

	/**
	 * Puts a new tower at a specified location on the map. This method is
	 * called by the Player to place a new tower during the running of the game.
	 * 
	 * @param towerID
	 *            String ID of Tower object to place
	 * @param location
	 *            Point2D representing location on the map
	 */
	public void placeTower(String towerID, Point2D location) {
		// loc param can probably be removed because the tower can just hold its
		// location to be
		// placed at
		Tower temp = (Tower) myGameElementFactory.getGameElement("Tower",
				towerID);
		...
		if (canPlace(temp, location)) {
			...
			System.out.println(temp.getGUID()
					+ " added to the node list in Layout");
		}
	}
	...
	/**
	 * Checks to see if a specific tower may be placed at a specific point on
	 * the map. This method can be used by the Player to check if a tower
	 * placement is valid.
	 * 
	 * @param tower
	 *            Tower object that is being tested for valid placement
	 * @param location
	 *            Point2D representing location on grid
	...
	public boolean canPlace(GameElement tower, Point2D location) {
		...
		Set<GameElement> possibleInteractions = myCollisionChecker
				.findCollisionsFor(tower);
		if (possibleInteractions.size() == 0)
			collisions = false;
		// tag checking
		boolean tags = true;
		myCollisionChecker.createQuadTree(this.getGridCells());
		Set<GameElement> possibleGridCells = myCollisionChecker
				.findCollisionsFor(tower);
	 ...
	 * 
	 * @param cell
	 *            GridCell object to check
	 * @param tower
	 *            Sprite object to check
	...
	/**
	 * Creates one or multiple new Enemy object and adds it to the map at the
	 * specified location.
	 * 
	 * @param enemyIDs
	 *            List<String> of IDs of Enemy objects to place
	 * @param location
	 *            Point2D representing location on grid
	 */
	...
	/**
	 * Creates a new Enemy object and adds it to the map at the specified
	 * location
	 * 
	 * @param enemyID
	 *            String ID of Enemy object to place
	 * @param location
	 *            Point2D representing location on grid
	 */
	...
	/**
	 * Creates one or multiple new Projectile object and adds it to the map at
	 * the specified location.
	 * 
	 * @param projectileIDs
	 *            List<String> of IDs of Projectile objects to place
	 * @param location
	 *            Point2D representing location on grid
	 */
	...
	/**
	 * Creates a new Projectile object and adds it to the map at the specified
	 * location
	 * 
	 * @param projectileID
	 *            String ID of Projectile object to place
	 * @param location
	 *            Point2D representing location on grid
	 */
	public void spawnProjectile(String projectileID, Point2D location,
			GameElement target) {
		Projectile proj = (Projectile) myGameElementFactory.getGameElement(
	...
	/**
	 * Updates the sprite locations and checks for collisions/apply effects of
	 * collision.
	 * 
	...
	/**
	 * Updates the positions of all sprites and spawns all new projectiles and
	 * enemies.
	 */
	private void updateSpriteLocations() {
		...
		myTowerList.forEach(p -> {
			...
			spawnMap.keySet().forEach(
					q -> spawnProjectile(spawnMap.get(q), (Point2D) q,
							p.getTarget()));
		});
		...
		myEnemyList.forEach(p -> {
			Map<Object, List<String>> spawnMap = p.update();
			spawnMap.keySet().forEach(
					q -> spawnEnemy(spawnMap.get(q), (Point2D) q));
		});

	}

	/**
	 * Checks for collisions between all objects and applies the effects of
	 * collision, such as changing velocity, or removing form the map.
	 */
	private void updateSpriteCollisions() {
		...
		for (Sprite sprite : this.getSprites()) {
			Set<GameElement> possibleInteractions = myCollisionChecker
					.findCollisionsFor(sprite);
			for (GameElement other : possibleInteractions) {
				// TODO determine how many interactions should be made to each
				// sprite
				myActionManager.applyAction(sprite, other);
				System.out.println(sprite.getPartType() + "\t"
						+ other.getPartType());
				if (myActionManager.isAction(sprite, other)) {
					boolean isProjectile = sprite.getPartType()
							.equalsIgnoreCase("Projectile");
					boolean isEnemy = other.getPartType().equalsIgnoreCase(
							"Enemy");
	...
	private void updateSpriteTargeting() {
		...
		for (Tower tower : myTowerList) {
			Set<GameElement> setOfTargetables = myCollisionChecker
					.findTargetable(tower);
	...
	/**
	 * Filters the list of whether or not the sprite is targetable by the tower
	 * 
	 * @param targetable
	 *            the set of GameElements that are in range of the tower
	 * @param tower
	 *            The tower that is firing
	 * @return Set of GameElements that can be targeted by the tower
	 */

	private Set<GameElement> filterTargets(Set<GameElement> targetable,
			Tower tower) {
		Projectile tester = (Projectile) myGameElementFactory.getGameElement(
	...
	/**
	 * Method used to initialize the game element factory with what possible
	 * game elements this game can create.
	 * 
	 * @param className
	 *            String of the class of the object, such as "Tower" or "Enemy"
	 * @param allObjects
	 *            Map<String, Map<String, Object>> object representing mapping
	 *            of GUID to parameter map
	 */
	public void initializeGameElement(String className,
			Map<String, Map<String, Object>> allObjects) {
