...
public class Layout extends GameElement implements Updateable {
	...
	public Layout(TileMap map){
		...
		for(int i = 0; i<tileMap.length; i++)
			for (int j = 0; j<tileMap[i].length; j++)
				terrainMap[i][j] = new GridCell(gridSize, tileMap[i][j].getTags()); //or some way of giving it the appropriate size and tags
		*/		
	}
	...
	public void placeTower(Sprite tower, Point2D loc){
		//loc param can probably be removed because the tower can just hold its location to be placed at
	...
	public boolean canPlace(Sprite tower, Point2D loc){
		...
		/*for (GridCell c: occupiedGridCells(tower))
			if(!tagsInCommon(c.getTags(),tower.getTags())) //if the cell has none of the tags of the tower then can't place so place is false{
				place = false;
				break;
			}*/
