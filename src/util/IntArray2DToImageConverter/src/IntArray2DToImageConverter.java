package util.IntArray2DToImageConverter.src;
...
public class IntArray2DToImageConverter {
    ...
    public static Image convert2DIntArrayToImage (int[][] sourceArray, double lengthSidePixel) {
        ...
        drawARGBValuesFromArrayIntoImageUsingPixelSideLength(sourceArray, returnImage,
                                                             lengthSidePixel);
    ...
    /**
     * Possible slight rounding with ints to doubles and back. Images will be marginally imperfect
     * due to JavaFX's WriteableImage using integer pixels
     * 
     * @param sourceArray
     * @param destinationImage
     * @param lengthSidePixel
     */
    private static void drawARGBValuesFromArrayIntoImageUsingPixelSideLength (int[][] sourceArray,
                                                                              WritableImage destinationImage,
                                                                              double lengthSidePixel) {

        PixelWriter pixelWriter = destinationImage.getPixelWriter();

        // Edited by Callie: Added in a -1 because otherwise there is an ArrayOutOfBoundsException
        System.out.println("destimg width " + destinationImage.getWidth());
        System.out.println("destimg height " + destinationImage.getHeight());
        for (int x = 0; x < destinationImage.getWidth() - 1; x++) {

            for (int y = 0; y < destinationImage.getHeight() - 1; y++) {

                pixelWriter
                        .setArgb(x,
                                 y,
                                 getIntARGBToDrawWithArrayWithImageXWithImageYWithLengthSidePixel(sourceArray,
                                                                                                  x,
                                                                                                  y,
                                                                                                  lengthSidePixel));
    ...
    private static int getIntARGBToDrawWithArrayWithImageXWithImageYWithLengthSidePixel (int[][] sourceArray,
                                                                                         int x,
                                                                                         int y,
                                                                                         double lengthSidePixel) {
        ...
        // System.out.println(currentXIndexToUse);
        // System.out.println(currentYIndexToUse);
        // System.out.println(sourceArray.length + " " + sourceArray[0].length);
