...
public abstract class SpriteView extends StackPane {
	...
	private void setupEditableContent() {
		...
		overlayErrorMessage = new Text(
				"Please check your parameters for errors.");
		...
		List<Setting> settings = ProjectReader
				.generateSettingsList(getSpriteType());
		...
		editableContent.getChildren().addAll(overlaySpriteNameDisplay,
				spriteImage, overlayErrorMessage, settingsObjects, buttons,
				saved);
	}
	...
	private void setupOverlayContent() {
		...
		Rectangle overlayBackground = new Rectangle(CONTENT_WIDTH,
				CONTENT_HEIGHT);
