...
public class ProjectReader {
	...
	private static final List<String> abstractEditors = SetHandler
			.listFromArray(new String[] { "Editor", "MainEditor",
					"PropertyEditor" });
	private static final List<String> mainEditors = SetHandler
			.listFromArray(new String[] { "LevelEditor", "MapEditor",
					"WaveEditor" });
	...
	public static void populateTabBar(MainEnvironment m,
			Dimension2D myDimensions, ResourceBundle myResources, Stage myStage) {
		...
		for (String s : ProjectReader.getOrderedTabList()) {
			if (tabsToCreate.keySet().contains(s)) {
				...
				try {
					e = (Editor) Class.forName(toCreate).getConstructor()
							.newInstance();
				} catch (InstantiationException e1) {
					System.err
							.println("Constructor Editor(Dimension2D.class, Stage.class) doesn't exist or was"
									+ "incorrectly called");
					...
					System.err
							.println("Error creating Editor object, Editor is currently null");
					e1.printStackTrace();
				}
